...
void printC(char temp[],string& digicode,int track){
 ...
 for(int i=0;i<keypad[digicode[track]-'0'].size();i++){
     temp[track]=keypad[digicode[track]-'0'][i];
    printC(temp,digicode,track+1);
 }
...
*/
    ...
    string digicode;
    cin>>digicode;
    char arr[digicode.size()];
   // printC(temp,digicode,0);
     printC(arr,digicode,0);
